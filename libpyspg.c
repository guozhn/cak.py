...
static struct PyMethodDef libpyspgMethods[] = {
...
static struct PyModuleDef libpyspgmodule = {
...
PyMODINIT_FUNC PyInit_libpyspg(void)
{
  return PyModule_Create(&libpyspgmodule);
}
...
static PyObject * get_dataset(PyObject *self, PyObject *args)
{
  ...
  PyList_SetItem(array, 1, PyUnicode_FromFormat("%s",dataset->international_symbol));
  PyList_SetItem(array, 2, PyUnicode_FromFormat("%s",dataset->hall_symbol));
...
static PyObject * get_spacegroup(PyObject *self, PyObject *args)
{
  ...
  PyTuple_SetItem(rslt, 0, PyLong_FromLong(num_spg));
  PyTuple_SetItem(rslt, 1, PyUnicode_FromFormat(symbol));
  return rslt;
}
...
static PyObject * get_pointgroup(PyObject *self, PyObject *args)
{
  ...
  array = PyList_New(3);
  PyList_SetItem(array, 0, PyUnicode_FromFormat(symbol));
...
static PyObject * get_ir_kpoints(PyObject *self, PyObject *args)
{
  ...
  return PyLong_FromLong((long) num_ir_kpt);
}
...
static PyObject * get_triplets_reciprocal_mesh(PyObject *self, PyObject *args)
{
  ...
  for (i = 0; i < spg_triplets->size; i++) {
    ...
    for (j = 0; j < 3; j++) {
      PyList_SetItem(tp, j,
		     PyLong_FromLong((long) spg_triplets->triplets[i][j]));
    }
    PyList_SetItem(triplets, i, tp);
    PyList_SetItem(weights, i, PyLong_FromLong((long) spg_triplets->weights[i]));
  }
  ...
  for (i = 0; i < num_grid; i++) {
    ...
    for (j = 0; j < 3; j++) {
      PyList_SetItem(tp, j,
		     PyLong_FromLong((long) spg_triplets->mesh_points[i][j]));
    }
...
static PyObject * extract_triplets_reciprocal_mesh_at_q(PyObject *self, PyObject *args)
{
  ...
  return PyLong_FromLong((long) num_triplets_at_q);
}
